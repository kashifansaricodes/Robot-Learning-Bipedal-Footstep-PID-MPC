...
cassie_mujoco_init(str.encode(ROOT_PATH + "/assets/cassie.xml"))
...
class CassieSim:
    ...
    def step_pd_without_estimation(self, u):
        y = cassie_out_t()
        cassie_sim_step_pd_without_estimation(self.c, y, u)
        return y

    def estimate_state(self, cassie_out):
        y = state_out_t()
        cassie_state_estimation(self.c, y, cassie_out)
        return y
    ...
    def get_foot_impact_forces(self):
        y = state_out_t()
        force = np.zeros(12)
    ...
    def get_dof_stiffness(self):
        ptr = cassie_sim_dof_stiffness(self.c)
        ret = np.zeros(self.nv)
        for i in range(self.nv):
            ret[i] = ptr[i]
        return ret
    ...
    def get_body_inertia(self):
        nbody = self.nbody * 3
        ptr = cassie_sim_body_inertia(self.c)
        ret = np.zeros(nbody)
        for i in range(nbody):
            ret[i] = ptr[i]
        return ret
    ...
    def get_driver_filter(self, fil):
        filter_array = (ctypes.c_int * 90)()
        cassie_sim_get_drive_filter(self.c, filter_array)
        for i in range(90):
            fil[i] = filter_array[i]

    def get_joint_filter(self, fil_x, fil_y):
        fil_x_array = (ctypes.c_double * 24)()
        fil_y_array = (ctypes.c_double * 18)()
        cassie_sim_get_joint_filter(self.c, fil_x_array, fil_y_array)
        for i in range(24):
            fil_x[i] = fil_x_array[i]
        for i in range(18):
            fil_y[i] = fil_y_array[i]

    def get_torque_delay(self, t):
        delay_array = (ctypes.c_double * 60)()
        cassie_sim_torque_delay(self.c, delay_array)
        for i in range(60):
            t[i] = delay_array[i]
    ...
    def set_dof_stiffness(self, data):
        c_arr = (ctypes.c_double * self.nv)()
        ...
        cassie_sim_set_dof_stiffness(self.c, c_arr)
    ...
    def set_body_inertia(self, data):
        nbody = self.nbody * 3
        c_arr = (ctypes.c_double * nbody)()
        ...
        cassie_sim_set_body_inertia(self.c, c_arr)
    ...
    def set_geom_name_pos(self, name, pos):
        pos_array = (ctypes.c_double * 3)()
        for i in range(3):
            pos_array[i] = pos[i]
        cassie_sim_set_geom_name_pos(self.c, name.encode(), pos_array)
...
class CassieVis:
    def __init__(self, c, modelfile, offscreen=False):
        # self.v = cassie_vis_init(c.c, modelfile.encode('utf-8'))
        self.v = cassie_vis_init(
            c.c, modelfile.encode("utf-8"), ctypes.c_bool(offscreen)
        )
...
class CassieUdp:
    ...
    # added for local communication
    def send_state_out(self, state):
        pack_state_out_t(state, self.inbuf)
        send_packet(self.sock, self.recvbuf, self.recvlen_pd, None, 0)

    # added for local communication
    def send_cassie_out(self, cassie_out):
        pack_cassie_out_t(cassie_out, self.inbuf)
        send_packet(self.sock, self.recvbuf, self.recvlen, None, 0)
    ...
    def recv_wait_u(self):
        nbytes = -1
        while nbytes != self.sendlen_pd:
            nbytes = get_newest_packet(
                self.sock, self.sendbuf, self.sendlen_pd, None, None
            )
        process_packet_header(self.packet_header_info, self.sendbuf, self.recvbuf)
        u_out = pd_in_t()
        unpack_pd_in_t(self.outbuf, u_out)
        return u_out

    def recv_newest_u(self):
        nbytes = get_newest_packet(self.sock, self.sendbuf, self.sendlen_pd, None, None)
        if nbytes != self.sendlen_pd:
            return None
        process_packet_header(self.packet_header_info, self.sendbuf, self.recvbuf)
        u_out = pd_in_t()
        unpack_pd_in_t(self.outbuf, u_out)
